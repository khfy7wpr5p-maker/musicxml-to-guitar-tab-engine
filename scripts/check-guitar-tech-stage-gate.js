...
function validateStageGate(state, { targetStage = 'PROD-TECH-03', expectedBaseSha = null } = {}) {
  const reasons = [];
  if (!state || state.documentType !== 'GuitarTechniqueProductionStageGateState') {
    ...
    if (typeof state.auditedMainSha !== 'string' || !/^[a-f0-9]{40}$/.test(state.auditedMainSha)) {
      reasons.push('AUDITED_MAIN_SHA_MISSING');
    }
    if (typeof state.auditReportSha256 !== 'string' || !/^[a-f0-9]{64}$/.test(state.auditReportSha256)) {
      reasons.push('AUDIT_REPORT_HASH_MISSING');
    }
    if (expectedBaseSha && state.auditedMainSha !== expectedBaseSha) {
      reasons.push('AUDITED_MAIN_SHA_STALE');
    }
  }
...
if (require.main === module) {
  ...
  const verdict = validateStageGate(readState(statePath), { targetStage, expectedBaseSha });
