...
const GUITARSET_OBSERVED_VOICING_SHADOW_VERSION = '1.0.0';
const EXPECTED_MODEL_ARTIFACT_SHA256 = '5d109e3b46ef286439f00ad6fa5885fc7bdf13e070974c49040c27b007461869';
...
class GuitarSetObservedVoicingShadowError extends EngineError {
  constructor(message, details = {}) {
    super(
      message,
      'INVALID_GUITARSET_OBSERVED_VOICING_SHADOW_INPUT',
      Object.freeze({ ...details }),
      'GuitarSetObservedVoicingShadowError',
    );
  }
}

function invalid(message, field, details = {}) {
  return new GuitarSetObservedVoicingShadowError(message, { field, ...details });
}
...
function validateModelArtifact(artifact) {
  ...
  if (
    ...
    throw invalid('Retained development artifact authority boundary drift.', 'modelArtifact');
  }
  if (
  ...
  if (
    ...
    throw invalid('Retained model pipeline drift.', 'modelArtifact.pipeline');
  }
  ...
  if (
    ...
    throw invalid('Retained model parameter dimensions drift.', 'modelArtifact.parameters');
  }
  ...
  if (transportSha256 !== EXPECTED_MODEL_TRANSPORT_SHA256) {
    throw invalid('Inference-relevant model transport digest drift.', 'modelArtifact.parameters', {
...
function canonicalizeTriples(candidate, field = 'candidate') {
  ...
  const triples = candidate.map((row, index) => {
    ...
    if (!Number.isInteger(fret) || fret < 0 || fret > DEFAULT_FRET_RANGE.maximumFret) {
      throw invalid('fret is outside the reviewed runtime 0..20 boundary.', `${field}[${index}][2]`);
    }
...
function createGuitarSetObservedVoicingFeatureVector(candidate) {
  const canonical = canonicalizeTriples(candidate);
  if (canonical.some((row) => row[2] > MODEL_MAX_FRET)) {
    throw invalid('Candidate lies outside the frozen 0..19 model domain.', 'candidate');
  }
  ...
  if (values.length !== FEATURE_COUNT || values.some((value) => !Number.isFinite(value))) {
    throw invalid('Frozen 28D GuitarSet feature vector is invalid.', 'candidate');
  }
...
function validateRuntimeCandidateModel(model) {
  ...
  if (
    ...
    || model.configuration.minimumFret !== DEFAULT_FRET_RANGE.minimumFret
    || model.configuration.maximumFret !== DEFAULT_FRET_RANGE.maximumFret
    || DEFAULT_FRET_RANGE.minimumFret !== 0
    || DEFAULT_FRET_RANGE.maximumFret !== 20
  ) {
...
function normalizeRuntimeGroup(group, groupIndex) {
  ...
  for (let candidateIndex = 0; candidateIndex < group.candidates.length; candidateIndex += 1) {
    ...
    normalized.push({ candidateId: candidate.candidateId, canonical });
  }
  return { targetMidis, candidates: normalized };
}

function createGuitarSetObservedVoicingShadowReport(voicingCandidateModel, modelArtifact) {
  const runtimeModel = validateRuntimeCandidateModel(voicingCandidateModel);
  const retainedModel = validateModelArtifact(modelArtifact);
  const groups = [];
  ...
  for (let groupIndex = 0; groupIndex < runtimeModel.groups.length; groupIndex += 1) {
    ...
    if (outOfDomain.length > 0) {
      ...
      groups.push({
        ...
        shadowScored: false,
        modelDomainComplete: false,
        outOfModelDomainCandidateCount: outOfDomain.length,
        topCandidateId: null,
        candidateScores: [],
      });
      continue;
    ...
    groups.push({
      ...base,
      status: 'SHADOW_SCORED_OFFLINE_NON_AUTHORITATIVE',
      shadowScored: true,
      modelDomainComplete: true,
      outOfModelDomainCandidateCount: 0,
  ...
  return deepFreeze({
    ...
    contractVersion: GUITARSET_OBSERVED_VOICING_SHADOW_VERSION,
