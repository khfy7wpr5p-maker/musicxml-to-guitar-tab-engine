...
function candidateFromSingleton(unit, position) {
  ...
  const signature = `singleton|${positionSignature(positions)}`;
  return Object.freeze({
    kind: 'SINGLETON',
    sourceGroupId: null,
...
function chooseBestShapeForVoicing(voicing, physicalVoicing, sourceGroupId) {
  ...
  for (const shape of voicing.shapeCandidates) {
    ...
    const signature = `${positionSignature(voicing.positions)}|${shapeSignature(shape)}`;
    candidates.push({ shape, verdict, signature });
  }
...
function candidatesFromGroup(unit, handoff) {
  ...
  for (const voicing of leftGroup.voicingCandidates) {
    ...
    const signature = `group|${unit.sourceGroupId}|${best.signature}`;
...
function buildIndexes(source, arrangement, grouping, reduction) {
  ...
  const decisionsById = new Map(arrangement.decisions.map((decision) => [decision.decisionId, decision]));
  const instructionsBySourceEventId = new Map(
...
function selectPath(unitCandidates, runtime) {
  let states = unitCandidates[0].map((candidate) => Object.freeze({
    candidate,
    previous: null,
    cost: Object.freeze([
      0,
  ...
  for (let unitIndex = 1; unitIndex < unitCandidates.length; unitIndex += 1) {
    ...
    for (let candidateIndex = 0; candidateIndex < unitCandidates[unitIndex].length; candidateIndex += 1) {
      checkpoint(runtime, 'deterministic-final-selection:path-candidate', { unitIndex, candidateIndex });
      const candidate = unitCandidates[unitIndex][candidateIndex];
      let best = null;
      for (const [anchor, previous] of bestByAnchor) {
        const state = {
          candidate,
          previous,
          cost: extendCost(previous.cost, candidate, anchor),
          pathKey: `${previous.pathKey}|${candidate.signature}`,
        };
        if (!best || compareStates(state, best) < 0) best = state;
      }
      nextStates[candidateIndex] = Object.freeze(best);
    }
    states = nextStates;
  }
...
function createDeterministicPolyphonicFinalSelection(sourceModel, arrangementDecisions, runtime = null) {
  ...
  const indexes = buildIndexes(source, arrangement, grouping, reduction);
  ...
  if (retainedInstructionCount === 0) {
    throw unsupported(
      'Final selection requires at least one retained output note.',
      'NO_RETAINED_OUTPUT_NOTES',
    );
  }
  ...
  const unitCandidates = units.map((unit, unitIndex) => {
    ...
    return buildUnitCandidates(unit, handoff);
  });
