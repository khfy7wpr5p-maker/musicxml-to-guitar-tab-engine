...
function assertScalarLeaf(node, allowedTexts, field, location) {
  ...
  if (
    ...
    throw unsupported(`${field} is outside the bounded 3:2/6:4/11:6/22:12 tuplet profile.`, {
      ...location,
      field,
      observedText,
    });
...
function parseExactTripletTimeModification(node, location) {
  ...
  const actualNotes = assertScalarLeaf(children[0], new Set(['3', '6', '11', '22']), 'actual-notes', location);
  ...
  if (!(
    ...
    throw unsupported('Only exact 3:2, 6:4, pinned 11:6, and pinned 22:12 tuplet relations are supported.', {
      ...location,
      actualNotes,
      normalNotes,
    });
