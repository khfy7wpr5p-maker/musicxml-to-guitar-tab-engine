...
test('runtime host rejects transport misuse before engine execution', async (t) => {
  ...
  const missingCommands = await json(await fetch(
  ...
  assert.equal(missingCommands.response.status, 400);
  assert.equal(missingCommands.payload.code, 'MISSING_EDIT_COMMANDS');
});
